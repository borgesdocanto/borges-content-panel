...
type Contenido = {
  ...
  ig_titulo: string; ig_descripcion: string; ig_hashtags: string
...
export default function Panel() {
  ...
  const VideoCard = ({ c }: { c: Contenido }) => (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, display: 'flex', overflow: 'hidden' }}>
      {/* Portada vertical 9:16 */}
      <div style={{ width: 90, flexShrink: 0, position: 'relative', background: 'var(--bg3)' }}>
        {c.portada_vertical_path ? (
          <img
            src={c.portada_vertical_path}
            alt="portada"
            style={{ width: 90, height: 160, objectFit: 'cover', objectPosition: 'center', display: 'block' }}
          />
        ) : (
          <div style={{ width: 90, height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, color: 'var(--text2)' }}>🎬</div>
        )}
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={key} title={`${meta.label}: ${pub ? 'Publicado' : 'Pendiente'}`} style={{
              ...
              {meta.emoji} {key.toUpperCase()} {pub ? '✓' : '○'}
            </div>
          ))}
        </div>
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <aside style={{ position: 'fixed', left: 0, top: 0, width: 220, height: '100vh', background: 'var(--bg2)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', zIndex: 100 }}>
        ...
        <div style={{ padding: '0 12px 8px' }}>
          ...
          <div onClick={logout} style={{ padding: '8px 12px', borderRadius: 8, cursor: 'pointer', textAlign: 'center', fontSize: 12, color: 'var(--text2)', background: 'var(--bg3)', marginBottom: 12 }}>
            Cerrar sesión
          </div>
        </div>
      </aside>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
                  ...
                  <Card>
                    ...
                    {contenidos.filter(c => {
                      const dias = (Date.now() - new Date(c.fecha_aprobacion).getTime()) / 86400000
                      return dias >= parseInt(config.dias_reposteo || '90') && (c.score_promedio || 0) >= parseFloat(config.score_reposteo || '7.5')
                    }).length === 0 && <div style={{ color: 'var(--text2)', fontSize: 13 }}>Los videos aptos aparecerán según tu configuración.</div>}
                  </Card>
                </div>
                <Card>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12 }}>
                    {[
                      ...
                      { label: 'Score <6',   count: contenidos.filter(c => (c.score_promedio||0) > 0 && (c.score_promedio||0) < 6).length, color: 'var(--red)' },
                    ].map(s => (
      ...
      {selectedContent && (
        <div onClick={() => setSelectedContent(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, width: 620, maxWidth: '100%', maxHeight: '88vh', overflowY: 'auto' }}>
            {/* Header */}
            <div style={{ display: 'flex', gap: 16, padding: '20px 20px 16px', borderBottom: '1px solid var(--border)', alignItems: 'flex-start' }}>
              {selectedContent.portada_vertical_path && (
                <img src={selectedContent.portada_vertical_path} alt="portada"
                  style={{ width: 72, height: 128, borderRadius: 8, objectFit: 'cover', objectPosition: 'center', flexShrink: 0 }} />
              )}
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {selectedContent.file_id_drive && (
                  <a href={`https://drive.google.com/file/d/${selectedContent.file_id_drive}/view`} target="_blank" rel="noreferrer"
                    style={{ fontSize: 12, color: 'var(--blue)', textDecoration: 'none' }}>▶ Ver video en Drive ↗</a>
                )}
              </div>
              ...
              {[
                ...
                <div key={r.meta.label} style={{ background: 'var(--bg3)', borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: `${r.meta.color}18`, borderBottom: `1px solid ${r.meta.color}33` }}>
                    <span style={{ fontSize: 15 }}>{r.meta.emoji}</span>
                  ...
                  <div style={{ padding: '10px 14px' }}>
                    ...
                    {r.desc && (
                      <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.7 }}>{r.desc}</div>
                    )}
                    {r.extra && (
                      <div style={{ fontSize: 12, color: 'var(--blue)', marginTop: 8, lineHeight: 1.6, wordBreak: 'break-word' }}>{r.extra}</div>
                    )}
                  </div>
                </div>
              ))}
